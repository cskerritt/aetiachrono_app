...
```bash
mkvirtualenv aetiachrono_app -p python3.11
...
Create a database named `aetiachrono_app`.

```
createdb aetiachrono_app
```
...
```bash
celery -A aetiachrono_app worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A aetiachrono_app worker -l INFO -B --pool=solo
```
